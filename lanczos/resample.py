...
def resample_with_wcs(
    ...
    if spline:
        ...
        R = targetwcs.pixelxy2radec(
            xx[np.newaxis, :] + 1, yy[:, np.newaxis] + 1
        )
        if len(R) == 3:
            ok = R[0]
            assert np.all(ok)
    ...
    # interpolate...
    ixo = np.arange(
        max(0, x0 - margin), min(W, x1 + margin + 1), dtype=intType
    )
    iyo = np.arange(
        max(0, y0 - margin), min(H, y1 + margin + 1), dtype=intType
    )
    ...
    if spline:
        ...
        R = targetwcs.pixelxy2radec(
            ixo[np.newaxis, :] + 1.0, iyo[:, np.newaxis] + 1.0
        )
        if len(R) == 3:
            # ok,ra,dec
            R = R[1:]
    ...
    if len(Limages):
        ...
        if cinterp:
            ...
            _lanczos_interpolate(
                L, ixi, iyi, dx, dy, laccs, Limages, table=table
            )
        rims = laccs
    else:
        rims = []
...
def _lanczos_interpolate(L, ixi, iyi, dx, dy, laccs, limages, table=True):
    """
    L: int, Lanczos order
